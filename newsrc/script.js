...
window.baseUrl = 'https://ada886.shop';
...
const get_client_token = () => {
  return new Promise(async (resolve, reject) => {
    try {
      const response = await fetch(baseUrl + "/get_client_token", {
      ...
      .then(() => {
          ...
          let paypal_buttons = paypal.Buttons({ // https://developer.paypal.com/sdk/js/reference
              ...
              createOrder: function(data, actions) { //https://developer.paypal.com/docs/api/orders/v2/#orders_create
                  return fetch(baseUrl + "/create_order", {
              ...
              onApprove: function(data, actions) {
                  ...
                  console.log(data);
                  return fetch(baseUrl + "/complete_order", {
                      method: "post", headers: { "Content-Type": "application/json; charset=utf-8" },
                      body: JSON.stringify({
                          "intent": intent,
                          "order_id": order_id
...
const handlePrice = (price) => {
  // 查找具有特定类名的元素
  const element = document.querySelector('.ms-label.ms-large.ms-action2.ms-light');

  // 更改元素的文本内容
  element.textContent = `${price} USD`;
};
init()
